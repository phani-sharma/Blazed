...
/// </summary>
public class RCC_CarControllerV3 : MonoBehaviourPun {
...
//		GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(10f, 1, 1);
	...
	void Inputs(){
		...
		case RCC_Settings.ControllerType.Keyboard:
			...
			handbrakeInput =RaceManager.Instance.RaceStarting?1:Input.GetKey(RCCSettings.handbrakeKB) ? 1f : 0f;
			...
			if(Input.GetKeyUp(KeyCode.R))
                {
					this.transform.position = GetComponent<PlayerInformation>().resetPoint.transform.position;
					this.transform.rotation = GetComponent<PlayerInformation>().resetPoint.transform.rotation;
    ...
    {
		if(!isNos)
		StartCoroutine(WaitNosEnd());
    }
	IEnumerator WaitNosEnd()
	{
			isNos = true;
	...
	public bool CheckisMine() => base.photonView.IsMine;	
} 
